...
# ── CONFIG ───────────────────────────────────────────────────────────────────
COURSE_ID        = "90" # Change course ID if needed
OPENROUTER_KEY   = "YOUR_OPENROUTER_KEY_HERE"
OPENROUTER_MODEL = "openrouter/free"  # Or a specific model e.g. "google/gemma-3n-e4b-it:free"
QUIZ_TARGET      = 50
TERMS_TARGET     = 20
DELAY            = 0.8
...
def ask_ai(prompt):
    try:
        ...
        if "choices" not in data:
            print("[OpenRouter error]", data.get("error", {}).get("message", data))
            return ""
        content = data["choices"][0]["message"].get("content")
        if not content:
            print("[OpenRouter error] empty response")
            return ""
